...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    if settings.scheduler_enabled:
        ...
        # Aquisição inicial bloqueante: garante GRIB (00/06/12/18) e METAR
        # disponíveis ANTES de o servidor começar a aceitar requests.
        try:
            summary = await scheduler_runner.initial_acquisition()
            logger.info("Aquisição inicial concluída: %s", summary)
        except Exception as e:
            logger.exception("Falha na aquisição inicial: %s", e)
...
def create_app() -> FastAPI:
    app = FastAPI(
        ...
        description=(
            ...
            "do modelo GFS (NOAA) com persistência SQLite. API REST + Interface Web."
        ),
    ...
    async def site_root():
        """Interface web interativa (mapas, animações, dashboard, METAR, análises)."""
